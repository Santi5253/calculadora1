    ...
    // Selecciona las opciones de radio y el índice de la diapositiva actual
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'ArrowRight') {
            currentIndex = (currentIndex + 1) % slides.length;  // Avanza a la siguiente diapositiva
            updateSlide(currentIndex);
        } else if (event.key === 'ArrowLeft') {
            currentIndex = (currentIndex - 1 + slides.length) % slides.length;  // Regresa a la diapositiva anterior
            updateSlide(currentIndex);
        }
    });
